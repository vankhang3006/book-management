...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto p-6">
        <h1 class="text-2xl font-bold mb-4 text-center">Thêm mới Sách</h1>
        ...
        <form action="{{ route('admin.books.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow-md">
            @csrf
            
            <!-- Tên sách -->
            <div class="mb-4">
                <label for="title" class="block text-gray-700 font-medium">Tên sách</label>
                <input type="text" id="title" name="title" value="{{ old('title') }}" class="w-full p-2 border border-gray-300 rounded" required>
                @error('title')
            ...
            <div class="mb-4">
                ...
                <input type="text" id="author" name="author" value="{{ old('author') }}" class="w-full p-2 border border-gray-300 rounded" required>
                @error('author')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea id="description" name="description" rows="4" class="w-full p-2 border border-gray-300 rounded">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
