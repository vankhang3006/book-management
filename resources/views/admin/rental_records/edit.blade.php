...
<head>
    ...
    <title>Chỉnh Sửa Bản ghi Thuê Sách</title>
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto p-6">
        <h1 class="text-2xl font-bold mb-4 text-center">Chỉnh Sửa Bản ghi Thuê Sách</h1>
        ...
        <form action="{{ route('admin.rental_records.update', $rentalRecord) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select name="book_copy_id" id="book_copy_id" class="block w-full">
                    @foreach ($bookCopies as $copy)
                        <option value="{{ $copy->id }}" {{ $rentalRecord->book_copy_id == $copy->id ? 'selected' : '' }}>
                            {{ $copy->book->title }} (Mã: {{ $copy->code }})
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="user_id" id="user_id" class="block w-full">
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" {{ $rentalRecord->user_id == $user->id ? 'selected' : '' }}>
                            {{ $user->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="date" id="rented_at" name="rented_at" 
                       value="{{ old('rented_at', $rentalRecord->rented_at) }}" 
                       class="block w-full">
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="date" id="due_date" name="due_date" 
                       value="{{ old('due_date', $rentalRecord->due_date) }}" 
                       class="block w-full">
            </div>
